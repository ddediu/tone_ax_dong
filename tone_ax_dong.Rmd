---
title: "Psychometric methods and Signal Detection Theory uncover subtle differences in the perception of tone contrasts in speakers of *Kam* (*Dong*)"
...
library(DiagrammeR); # plotting networks
library(png); # needed by lavaanPlot::embed_plot_pdf()
...
supp_mats <- FALSE; # to avoid conflicts with the actual supplementary information, we use a simple numbering system here
...
- *Figures:*
  + [Figure 1. Distribution of gender.](#Fig1)
  + [Figure 2. Distribution of age.](#Fig2)
  + [Figure 4. Distribution of years of musical traning.](#Fig4)
  + [Figures 5-6. Distribution of years of education by gender and by location.](#Fig5)
  + [Figure 7. Predictive plot of the linear regression of education on gender, age and their interaction.](#Fig7)
  + [Figures 9-11. PCA of working memory.](#Fig9)
  + [Figures 12-14. EFA of working memory.](#Fig12)
  + [Figure 15. CFA of working memory.](#Fig15)
  + [Figure 16. Normalized working memory by gender and age.](#Fig16)
  + [Figure 17. Number of correct responses on the tone task (original dataset).](#Fig17)
  + [Figures 18-21. Correlation matrix, its histogram, hierarchical clustering, and average correlations for the tone task (original dataset).](#Fig18)
  + [Figure 24. Guttman errors for the tone task (original dataset).](#Fig24)
  + [Figures 25-27. PCA of the tone task (original dataset).](#Fig25)
  + [Figures 28-30. EFA of the tone task (original dataset).](#Fig28)
  + [Figures 31-35. Correlation matrix, its histogram, hierarchical clustering, and average correlations for the tone task (recoded dataset).](#Fig31)
  + [Figures 36-38. PCA of the tone task (recoded dataset).](#Fig36)
  + [Figures 39-41. EFA of the tone task (recoded dataset).](#Fig39)
  + [Figures 42-43. Histograms and correlations heatmaps (with clustering) for pcr on the three datasets.](#Fig42)
  + [Figures 46-47. Histograms and correlations heatmaps (with clustering) for SDT-deroved measures on the three datasets.](#Fig46)
  + [Figure 55. K-means clustering for the tone task (original dataset).](#Fig55)
  + [Figure 56. % different vs % same for the tone task clusters.](#Fig56)
  + [Figures 57-58. Correlations heatmaps (with clustering) for all measures of interest.](#Fig57)
  + [Figure 60. Mediation gender → education → working memory.](#Fig60)
  + [Figure 61. Mediation age → education → working memory.](#Fig61)
  + [Figures 62-64. Multiple regression of pcr (recoded dataset).](#Fig62)
  + [Figure 65. Mediation gender → education → tone (recoded dataset).](#Fig65)
  + [Figure 66. Mediation age → education → tone (recoded dataset).](#Fig66)
  + [Figure 67. Mediation location → education → tone (recoded dataset).](#Fig67)
  + [Figure 68. Mediation education, age, gender → working memory → tone (recoded dataset).](#Fig68)
  + [Figures 70-77. Corresponding analyses for d' (recoded dataset).](#Fig70)
  + [Figures 94-99. Comparing the complex path models for % correct responses and d' on the original, recoded and removed datasets.](#Fig94)

- *Tables:*
  + [Table 1. The working memory trials.](#Tab1)
  + [Tables 5-6. Tone task stimuli.](#Tab5)
  + [Table 7. Frequencies of correct responses on the tone task (original dataset).](#Tab7)
  + [Tables 9-10. Item homogeneities for the tone task (original dataset).](#Tab9)
  + [Table 11. aisp for the tone task (original dataset).](#Tab11)
  + [Table 12. Frequencies of correct responses on the tone task (recoded dataset).](#Tab12)
  + [Tables 13-14. Item homogeneities for the tone task (recoded dataset).](#Tab13)
  + [Table 15. aisp for the tone task (recoded dataset).](#Tab15)
  + [Tables 16-17. Item homogeneities for the tone task (reduced dataset).](#Tab16)
  + [Table 19. aisp for the tone task (reduced dataset).](#Tab19)
  + [Table 24. W and non-W item correlations.](#Tab24)
  + [Table 26. Comparisons between measures and datasets.](#Tab26)
  + [Table 27. Examples of response patterns detectable by SDT but not by % correct responses.](#Tab27)


# Introduction

This document (irrespective of its format, most probably `HTML` or `PDF`) resulted from compiling the corresponding `Rmarkdown` script and contains all the results and plots supporting the paper (referred to as the **Analysis reports** in the main paper).
This `Rmarkdown` script also generates the (panels) of the supplementary figures and the content of the supplementary tables contained in the paper's **Supplementary Information**.
...
```{r}
# combine the covariates plots into a single figure for the paper:
p1 <- ggplot(d_ppts, aes(x=gender, fill=gender, color=gender)) + geom_bar(alpha=0.25) + 
  scale_fill_manual(values=c("F"="red", "M"="blue")) + scale_color_manual(values=c("F"="red", "M"="blue")) + 
  ggtitle("A"); 
p2 <- ggplot(d_ppts, aes(x=age, fill=gender, color=gender)) + geom_density(alpha=0.25) + geom_density(data=d_ppts, aes(x=age), alpha=0.5, color="black", fill=NA, linewidth=1.0) + 
  scale_fill_manual(values=c("F"="red", "M"="blue")) + scale_color_manual(values=c("F"="red", "M"="blue")) + 
  ggtitle("B");
p3 <- ggplot(d_all, aes(x=music_years, fill=gender, color=gender)) + 
  geom_histogram(alpha=0.25) + 
  scale_fill_manual(values=c("F"="red", "M"="blue")) + scale_color_manual(values=c("F"="red", "M"="blue")) + 
  ggtitle("C") + xlab("years of formal muscial training") +
  NULL;
p4 <- ggplot(d_all, aes(x=education_years, fill=gender, color=gender)) + 
  geom_histogram(alpha=0.25) + 
  scale_fill_manual(values=c("F"="red", "M"="blue")) + scale_color_manual(values=c("F"="red", "M"="blue")) + 
  ggtitle("D") + xlab("years of formal education") + 
  NULL;

tiff("./paper/figures/Fig2.tiff", units="in", width=2*3, height=2*3, res=600, compression="lzw", type="cairo");
grid.arrange(p1, p2, p3, p4, ncol=2);
dev.off();
             
cairo_ps("./paper/figures/Fig2.eps", width=2*3, height=2*3, onefile=TRUE, fallback_resolution=600);
grid.arrange(p1, p2, p3, p4, ncol=2);
dev.off();
             
jpeg("./paper/figures/Fig2.jpg", units="in", width=2*3, height=2*3, quality=85, res=300);
grid.arrange(p1, p2, p3, p4, ncol=2);
dev.off();

# regression:
tiff("./paper/figures/FigS1.tiff", units="in", width=1*5+0.25, height=1*5, res=600, compression="lzw", type="cairo");
sjPlot::plot_model(m_educ, type="pred", terms=c("age [all]", "gender")) + ylab("years of formal education") + ggtitle(NULL);
dev.off();

#cairo_ps("./paper/figures/FigS1.eps", width=1*5+0.25, height=1*5, onefile=TRUE, fallback_resolution=600);
#sjPlot::plot_model(m_educ, type="pred", terms=c("age [all]", "gender")) + ylab("years of formal education") + ggtitle(NULL);
#dev.off();

jpeg("./paper/figures/FigS1.jpg", units="in", width=1*5+0.25, height=1*5,  quality=85, res=300);
sjPlot::plot_model(m_educ, type="pred", terms=c("age [all]", "gender")) + ylab("years of formal education") + ggtitle(NULL);
dev.off();
```
...
```{r}
# for paper:
p1 <- fviz_eig(wm_pca) + ylab("% explained variance") + xlab(expression("PC"[i])) + ggtitle("A");

tiff("./paper/figures/FigS3A.tiff", units="in", width=1*5, height=1*5, res=600, compression="lzw", type="cairo");
p1;
dev.off();
```
...
```{r}
# for paper:
tiff("./paper/figures/FigS3B.tiff", units="in", width=1*5, height=1*5, res=600, compression="lzw", type="cairo");
fa.parallel(d, fa="fa", n.iter=100, error.bars=TRUE, main="B");
dev.off();
```
...
```{r}
# for paper:
#tiff("./paper/figures/FigS3C.tiff", units="in", width=1*5, height=1*5, res=600, compression="lzw", type="cairo");
p1 <- lavaanPlot(model=wm_cfa_1f_fit, coefs=TRUE, sig=1.00, stand=TRUE, covs=TRUE, stars=c("regress", "latent", "covs"), edge_options=list(color="gray"));
tmp <- embed_plot_pdf(p1, "./paper/figures/FigS3C.pdf");
#dev.off();
```
...
# For paper:
p1 <- ggplot(d_all, aes(x=wm_norm, fill=gender, color=gender)) + 
               geom_density(alpha=0.25) + geom_density(data=d_all, aes(x=wm_norm), alpha=0.5, color="black", fill=NA, linewidth=1.0) + xlab("normalized working memory total score") + 
               scale_fill_manual(values=c("F"="red", "M"="blue")) + scale_color_manual(values=c("F"="red", "M"="blue")) + xlim(c(0,1)) + ggtitle("A") + 
               NULL;
p2 <- ggplot(d_all, aes(x=age, y=wm_norm, color=gender)) + 
               geom_point(alpha=0.5) + geom_smooth(method="lm") + 
               scale_color_manual(values=c("F"="red", "M"="blue")) + ylim(c(0,1)) + ylab("normalized working memory total score") + xlab("age") + ggtitle("B") + 
               NULL;
p3 <- ggplot(d_all, aes(x=education_years, y=wm_norm, color=gender)) + 
               geom_point(alpha=0.5) + geom_smooth(method="lm") + 
               scale_color_manual(values=c("F"="red", "M"="blue")) + ylim(c(0,1)) + xlab("years of formal education") + ylab("normalized working memory total score") + ggtitle("C") + 
               NULL;

tiff("./paper/figures/FigS4.tiff", units="in", width=3*5.25, height=1*5, res=600, compression="lzw", type="cairo");
grid.arrange(p1, p2, p3, ncol=3);
dev.off();

jpeg("./paper/figures/FigS4.jpg", units="in", width=3*5.25, height=1*5, ,  quality=85, res=300);
grid.arrange(p1, p2, p3, ncol=3);
dev.off();
```
...
```{r}
# add the is_real_word info for printing/plotting:
d_itmes_real_words <- union(d_same_items$item_short1[ d_same_items$isword1 == "yes" ], d_different_items$item_short1[ d_different_items$isword1 == "yes" | d_different_items$isword2 == "yes" ] );
```
...
```{r}
kable(d_tone_wide_items_order %>% 
        dplyr::mutate(YESPercentage = sprintf("%.1f%%",YESPercentage*100)) %>%
        dplyr::mutate(Items = paste0(Items, ifelse(substring(Items, 1, nchar(Items)-1) %in% d_itmes_real_words, "*", ""))), 
      row.names=FALSE, col.names=c("Item (short name)", "# correct responses", "% correct responses"), align=c("l", "r", "r"), 
      caption=capTab("Frequencies of 'yes' responses (the items are ordered by % correct responses). The star (*) denotes those itmes that contain real words in the language."));
```
...
barplot(d_tone_wide_items_order[,"YESCount"], 
        ...
        cex.lab=0.8,
        cex.axis=0.5,
        names.arg=vapply(d_tone_wide_items_order[, "Items"], function(s) paste0(s, ifelse(substring(s, 1, nchar(s)-1) %in% d_itmes_real_words, "*", "")), character(1)), 
        las=2, 
        cex.names=0.8);
# dev.off()
```
...
barplot(bluesqs_mean, 
        ...
        cex.lab=0.8,
        cex.axis=0.5,
        names.arg=vapply(names(bluesqs_mean), function(s) paste0(s, ifelse(substring(s, 1, nchar(s)-1) %in% d_itmes_real_words, "*", "")), character(1)), 
        las=2, 
        cex.names=0.8);
```
...
# for the paper:
# barplot of endorsement frequencies (number of respondents with affirmative answers)
#tiff("./paper/figures/FigS5.tif", width=2*4, height=1*12, units="in", res=600, compression="lzw");
cairo_ps("./paper/figures/FigS5.eps", width=3*4, height=1*12, onefile=TRUE);
par(mfrow=c(1,3), oma=c(0,3,0,0), mar=c(3,2,1,1))
barplot(d_tone_wide_items_order[,"YESCount"], 
        horiz = TRUE,
        main = "A",
        ylab="", 
        ...
        las=2, 
        cex.names=0.5);
bluesqs_mean_for_plotting <- bluesqs_mean[ d_tone_wide_items_order[, "Items"] ];
barplot(bluesqs_mean_for_plotting, 
        horiz = TRUE,
        col = ifelse(bluesqs_mean_for_plotting < 0, "blue", "red"),
        main = "B",
        ylab="", 
        xlab="Mean tetrachoric correlation", 
        cex.lab=0.8,
        cex.axis=0.5,
        #names.arg=vapply(names(bluesqs_mean_for_plotting), function(s) paste0(s, ifelse(substring(s, 1, nchar(s)-1) %in% d_itmes_real_words, "*", "")), character(1)), 
        names.arg=rep("",length(bluesqs_mean_for_plotting)),
        las=2, 
        cex.names=0.8);
par(cex=0.4, cex.main=2); plot(as.dendrogram(items_clust), horiz=TRUE, main="C"); par(cex=1.0); 
dev.off();

jpeg("./paper/figures/FigS5.jpg", width=3*4, height=1*12, units="in", quality=85, res=300);
par(mfrow=c(1,3), oma=c(0,3,0,0), mar=c(3,2,1,1))
barplot(d_tone_wide_items_order[,"YESCount"], 
        horiz = TRUE,
        main = "A",
...
cat(paste0(rev(vapply(d_tone_wide_items_order[, "Items"], function(s) paste0(s, ifelse(substring(s, 1, nchar(s)-1) %in% d_itmes_real_words, "*", "")), character(1))), collapse=", "));
```
...
kable(d_special_items %>% 
        dplyr::mutate(YESPercentage = sprintf("%.1f%%",YESPercentage*100), mean_rho=round(mean_rho,2)) %>%
        dplyr::mutate(Items = paste0(Items, ifelse(substring(Items, 1, nchar(Items)-1) %in% d_itmes_real_words, "*", ""))), 
      row.names=FALSE, col.names=c("Item (short name)", "mean tetra. corr.", "# correct responses", "% correct responses"), align=c("l", "r", "r", "r"), 
...
Apparently, these "incorrect" responses persisted even when some of the participants were provided with explicit feedback by the experimenters (Manxiang Wu, p.c.) suggesting that these perceptions are "real" and not due to inattention or fatigue.
...
```{r}
# Are the real words enriched or impoverished among the weird items?
d_all_items <- data.frame("item"=sort(union(d_same_items$item_short1, d_different_items$item_short1)), "is.word"=NA, "is.weird"=NA, "is.same"=NA);
d_all_items$is.word <- (d_all_items$item %in% d_itmes_real_words);
d_all_items$is.weird <- (d_all_items$item %in% substring(weird_items, 1, nchar(weird_items)-1));
d_all_items$is.same <- (nchar(d_all_items$item) == 2);
pander(round(100*prop.table(ftable(d_all_items[,c("is.same", "is.word", "is.weird")])),1));
fisher.test(table(d_all_items[ !d_all_items$is.same, c("is.word", "is.weird" )])); # p=0.64
```
...
# -> so the repetitions does not seem to matter (but the same has better overall performance than different)
...
```{r results='hide'}
# for the paper:
tiff("./paper/figures/FigS6A.tiff", units="in", width=1*5, height=1*5, res=600, compression="lzw", type="cairo");
fviz_eig(t_pca) + ylab("% explained variance") + xlab(expression("PC"[i])) + ggtitle("A");
dev.off();

tiff("./paper/figures/FigS6B.tiff", units="in", width=1*5, height=1*5, res=600, compression="lzw", type="cairo");
fa.parallel(d, fa="fa", n.iter=100, error.bars=TRUE, main="B");
dev.off();

tiff("./paper/figures/FigS6C.tiff", units="in", width=1*10, height=1*10, res=600, compression="lzw", type="cairo");
fviz_pca_var(t_pca,
             col.var = "contrib", # Color by contributions to the PC
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE     # Avoid text overlapping
...
# for the paper:
# barplot of endorsement frequencies (number of respondents with affirmative answers)
#tiff("./paper/figures/FigS8.tif", width=2*4, height=1*12, units="in", res=600, compression="lzw");
cairo_ps("./paper/figures/FigS8.eps", width=3*4, height=1*12, onefile=TRUE);
par(mfrow=c(1,3), oma=c(0,3,0,0), mar=c(3,2,1,1))
barplot(d_tone_wide_items_order_flip[,"YESCount"], 
        horiz = TRUE,
        main = "A",
        ...
        cex.lab=0.8,
        cex.axis=0.5,
        names.arg=vapply(d_tone_wide_items_order_flip[, "Items"], function(s) paste0(s, ifelse(substring(s, 1, nchar(s)-1) %in% d_itmes_real_words, "*", "")), character(1)), 
        las=2, 
        cex.names=0.5);
bluesqs_flip_mean_for_plotting <- bluesqs_flip_mean[ d_tone_wide_items_order_flip[, "Items"] ];
barplot(bluesqs_flip_mean_for_plotting, 
        horiz = TRUE,
        col = ifelse(bluesqs_flip_mean_for_plotting < 0, "blue", "red"),
        main = "B",
        ylab="", 
        xlab="Mean tetrachoric correlation", 
        cex.lab=0.8,
        cex.axis=0.5,
        #names.arg=vapply(names(bluesqs_flip_mean_for_plotting), function(s) paste0(s, ifelse(substring(s, 1, nchar(s)-1) %in% d_itmes_real_words, "*", "")), character(1)), 
        names.arg=rep("",length(bluesqs_flip_mean_for_plotting)),
        las=2, 
        cex.names=0.8);
par(cex=0.4, cex.main=2); plot(as.dendrogram(items_clust), horiz=TRUE, main="C"); par(cex=1.0); 
dev.off();

jpeg("./paper/figures/FigS8.jpg", width=3*4, height=1*12, units="in", quality=85, res=300);
par(mfrow=c(1,3), oma=c(0,3,0,0), mar=c(3,2,1,1))
barplot(d_tone_wide_items_order_flip[,"YESCount"], 
        horiz = TRUE,
        main = "A",
...
cat(paste0(rev(vapply(d_tone_wide_items_order_flip[, "Items"], function(s) paste0(s, ifelse(substring(s, 1, nchar(s)-1) %in% d_itmes_real_words, "*", "")), character(1))), collapse=", "));
```
...
```{r results='hide'}
# for the paper:
tiff("./paper/figures/FigS9A.tiff", units="in", width=1*5, height=1*5, res=600, compression="lzw", type="cairo");
fviz_eig(t_pca_flip) + ylab("% explained variance") + xlab(expression("PC"[i])) + ggtitle("A");
dev.off();

tiff("./paper/figures/FigS9B.tiff", units="in", width=1*5, height=1*5, res=600, compression="lzw", type="cairo");
fa.parallel(d, fa="fa", n.iter=100, error.bars=TRUE, main="B");
dev.off();

tiff("./paper/figures/FigS9C.tiff", units="in", width=1*10, height=1*10, res=600, compression="lzw", type="cairo");
fviz_pca_var(t_pca_flip,
             col.var = "contrib", # Color by contributions to the PC
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE     # Avoid text overlapping
...
```{r}
# Compare PCA and EFA between the original and flipped conditions:
s <- (abs(t_pca_flip$rotation[,1] - t_pca$rotation[,1]) > 1e-5) | (abs(t_pca_flip$rotation[,2] - t_pca$rotation[,2]) > 1e-5);
tmp <- merge(t_pca$rotation[s,1:2], t_pca_flip$rotation[s,1:2], by=0, all=TRUE, suffixes=c(".original", ".flipped")); tmp <- tmp[,c(1,2,4,3,5)];
kable(tmp, digits=2, caption="Differences in PC loadings between the original and 'flipped' datasets (showing only the itmes where the loadings are more different than 10^-5^.");
#cor.test(tmp[,2], tmp[,3]); cor.test(tmp[,4], tmp[,5]);

s <- (abs(t_fa_2f$loadings[,1] - t_fa_2f_flip$loadings[,1]) > 1e-5) | (abs(t_fa_2f$loadings[,2] - t_fa_2f_flip$loadings[,2]) > 1e-5);
tmp <- merge(t_fa_2f$loadings[s,1:2], t_fa_2f_flip$loadings[s,1:2], by=0, all=TRUE, suffixes=c(".original", ".flipped")); tmp <- tmp[,c(1,2,4,3,5)];
kable(tmp, digits=2, caption="Differences in EFA loadings between the original and 'flipped' datasets (showing only the itmes where the loadings are more different than 10^-5^.");
#cor.test(tmp[,2], tmp[,3]); cor.test(tmp[,4], tmp[,5]);
```
...
However, here there is no relationship between the two % correct measures, with a significant positive prediction of % correct of the "weird" items from the  "non-weird" ones (`r sprintf("*β*=%.2f±%.2f, *p*=%s", (tmp <- summary(m_3_3)$coefficients)["pc_diff_remv", "Estimate"], tmp["pc_diff_remv", "Std. Error"], scinot(tmp["pc_diff_remv", "Pr(>|t|)"]))`).
...
```{r}
# for paper:
#tiff("./paper/figures/FigS7.tiff", units="in", width=2*5, height=2*5, res=600, compression="lzw", type="cairo");
cairo_ps("./paper/figures/FigS7.eps", width=2*5, height=2*5, onefile=TRUE);
grid.arrange(
  # k=2:
  fviz_cluster(km2, data=d, labelsize=9, geom="point", main="2 clusters") + 
    geom_smooth(aes(color=cluster), method="lm") + #geom_smooth(aes(color=cluster), method="loess") + 
    ...
    scale_fill_manual(values = colors_k2, label = labels_k2) + 
    scale_shape_manual(values = shape_k2, label = labels_k2) +
    xlab("% 'different' remove") + ylab("% 'different' weird") + 
...
jpeg("./paper/figures/FigS7.jpg", units="in", width=2*5, height=2*5, quality=85, res=300);
grid.arrange(
  # k=2:
  fviz_cluster(km2, data=d, labelsize=9, geom="point", main="2 clusters") + 
    geom_smooth(aes(color=cluster), method="lm") + #geom_smooth(aes(color=cluster), method="loess") + 
...
```{r}
# for the paper:
cairo_ps("./paper/figures/FigS10.eps", width=1*10, height=1*10, onefile=TRUE, fallback_resolution=600);
gplots::heatmap.2(tmp, cellnote=round(tmp,2), notecol="black", margins=c(12,12), 
                  labRow=c("age", "years education", "gender (F:M)", "location (A:B)", "working memory", 
                           "% total (original)", "% same (original)", "% different (original)", "d' (original)", "c (original)",
                           "% total (recode)",   "% same (recode)",   "% different (recode)",   "d' (recode)",   "c (recode)"), 
...
jpeg("./paper/figures/FigS10.jpg", units="in", width=1*10, height=1*10, quality=85, res=300);
gplots::heatmap.2(tmp, cellnote=round(tmp,2), notecol="black", margins=c(12,12), 
                  labRow=c("age", "years education", "gender (F:M)", "location (A:B)", "working memory", 
                           "% total (original)", "% same (original)", "% different (original)", "d' (original)", "c (original)",
                           "% total (recode)",   "% same (recode)",   "% different (recode)",   "d' (recode)",   "c (recode)"), 
    ...
    !file.exists("./cached_results/wm_task_results.rds") ) # computationally expensive
{
  ...
  wm_task_results <- list("icc_fam"=icc_wm__fam, "m_gen"=summary(m_wm__gen), "m_location"=summary(m_wm__location), # checks
                          "multreg"=summary(m_wm), "vif"=m_wm_vif, # multiple regression model
...
Moreover, the VIF (Variance Inflation Factors) of the retained predictors are:
```{r}
pander(wm_task_results$vif);
```
which, using a very conservative cut-off point of 3, suggest that there is no multicollinearity.
...
# med_gender__education:
g <- plot(wm_task_results$med_gender__education$piecewise$model, 
          node_attrs = data.frame(shape = "circle", color = "black", fillcolor = "white", fixedsize=FALSE, fontsize=6), 
          edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6), 
          return=TRUE);
g_std <- coefs(wm_task_results$med_gender__education$piecewise$model, standardize="scale"); # get the standardized coefficients and p-values
g_nodes <- get_node_df(g);
g_edges <- get_edge_df(g);
g_edges <- do.call(rbind, lapply(1:nrow(g_edges), function(i)
{
  g_estim <- g_std[ g_std$Response == g_nodes$label[g_edges$to[i]] & g_std$Predictor == g_nodes$label[g_edges$from[i]], ];
  cbind(g_edges[i,], "estim.nstd"=g_estim$Estimate[1], "estim.std"=g_estim$Std.Estimate[1], "p.val"=g_estim$P.Value[1]);
}));
g_edges$estim <- ifelse(g_edges$estim.std != "-", as.numeric(g_edges$estim.std), as.numeric(g_edges$estim.nstd));
g_edges$label <- sprintf("%s%.1g%s%s",
                         ifelse(g_edges$estim.std == "-", "[", ""),
                         g_edges$estim, 
                         ifelse(g_edges$estim.std == "-", "]", ""),
                         ifelse(g_edges$p.val<0.001, "***", ifelse(g_edges$p.val<0.01, "**", ifelse(g_edges$p.val<0.05, "*", ""))));
g <- set_node_attrs(g, "label",     c("educ",  "wm",   "gender[M]"));
g <- set_node_attrs(g, "fillcolor", c("white", "white", "white"));
g <- set_node_attrs(g, "fontcolor", c("black", "black", "black"));
g <- set_node_attrs(g, "color",     c("black", "black", "black"));
g <- set_edge_attrs(g, "color",     ifelse(g_edges$style == "solid", ifelse(as.numeric(g_edges$estim) > 0, "darkorange", "deepskyblue"), "gray90"));
g <- set_edge_attrs(g, "fontcolor", ifelse(g_edges$style == "solid", ifelse(as.numeric(g_edges$estim) > 0, "firebrick", "navyblue"), "gray60"));
g <- set_edge_attrs(g, "label",     ifelse(g_edges$style == "solid", g_edges$label, ""));
render_graph(g);
DiagrammeR::export_graph(g, file_name="./paper/figures/Fig3A.pdf", file_type="pdf");


# med_age__education:
g <- plot(wm_task_results$med_age__education$piecewise$model, 
          node_attrs = data.frame(shape = "circle", color = "black", fillcolor = "white", fixedsize=FALSE, fontsize=6), 
          edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6), 
          return=TRUE);
g_std <- coefs(wm_task_results$med_age__education$piecewise$model, standardize="scale"); # get the standardized coefficients and p-values
g_nodes <- get_node_df(g);
g_edges <- get_edge_df(g);
g_edges <- do.call(rbind, lapply(1:nrow(g_edges), function(i)
{
...
g_edges$label <- sprintf("%s%.1g%s%s",
                         ...
                         ifelse(g_edges$estim.std == "-", "]", ""),
                         ifelse(g_edges$p.val<0.001, "***", ifelse(g_edges$p.val<0.01, "**", ifelse(g_edges$p.val<0.05, "*", ""))));
g <- set_node_attrs(g, "label",     c("educ",  "wm",   "age"));
g <- set_node_attrs(g, "fillcolor", c("white", "white", "white"));
g <- set_node_attrs(g, "fontcolor", c("black", "black", "black"));
g <- set_node_attrs(g, "color",     c("black", "black", "black"));
g <- set_edge_attrs(g, "color",     ifelse(g_edges$style == "solid", ifelse(as.numeric(g_edges$estim) > 0, "darkorange", "deepskyblue"), "gray90"));
...
g <- set_edge_attrs(g, "label",     ifelse(g_edges$style == "solid", g_edges$label, ""));
render_graph(g);
DiagrammeR::export_graph(g, file_name="./paper/figures/Fig3B.pdf", file_type="pdf");
```
    ...
    !file.exists("./cached_results/tone_pcr_results.rds") ) # computationally expensive
{
  ...
  m_pcr_vif <- car::vif(lm(formula(m_pcr), data=d), type="predictor"); # refit as a linear model as glmmTMB does not currently work and switching to linear should not be an issue in this case: all < 3 except for gender (11.4)
  ...
  ## working memory has a direct effect besides what education, age and gender would predict:
  ...
  # multiple regression:
  m_pcr_full_ng <- glmmTMB(pcr ~ age * education_years * location_ab * wm_norm, family=beta_family(), control=beta_ctrl, data=d); summary(m_pcr_full_ng);
  m_pcr_ng <- update(m_pcr_full_ng, . ~ . -
                       age:education_years:location_ab:wm_norm - # 4-way interaction
                       education_years:location_ab:wm_norm - age:education_years:wm_norm - age:education_years:location_ab - age:location_ab:wm_norm - # 3-way interactions
                       age:wm_norm - age:location_ab - education_years:wm_norm - location_ab:wm_norm - education_years:location_ab # 2-way interactions
  ); summary(m_pcr_ng); (anova_m_pcr_ng = anova(m_pcr_ng, m_pcr_full_ng));
  sjPlot::plot_model(m_pcr_ng, transform=NULL); 
  sjPlot::plot_model(m_pcr_ng, type="pred", terms=c("age", "education_years"));
  m_pcr_vif_ng <- car::vif(lm(formula(m_pcr_ng), data=d), type="predictor"); # refit as a linear model as glmmTMB does not currently work and switching to linear should not be an issue in this case: all < 3
  
  ## working memory has a direct effect besides what education and age would predict:
  med_pcr__mediator_ng <-  lm(wm_norm ~ education_years + age, data=d); summary(med_pcr__mediator_ng);
  med_pcr__outcome_ng  <- lm(pcr ~ wm_norm + education_years + age, data=d); summary(med_pcr__outcome_ng); # mediation::mediate cannot do beta regression so let's use linear regression instead...
  summary(glmmTMB(pcr ~ wm_norm + education_years + age, family=beta_family(), control=beta_ctrl, data=d)); # ... and the estimates and p-values are very similar between the two
  med_pcr_wm_ng <- mediation::mediate(med_pcr__mediator_ng, med_pcr__outcome_ng, mediator="wm_norm", treat="education_years", outcome="pcr", sims=10000, robustSE=TRUE); summary(med_pcr_wm_ng); plot(med_pcr_wm_ng);

  psem_pcr_wm_ng <- piecewiseSEM::psem( # with indirect effect (aka partial mediation)
    lm(wm_norm ~ education_years + age, data=d),
    lm(pcr ~ wm_norm + education_years + age, data=d),
    data=d);
  (summary_psem_pcr_wm_ng <- summary(psem_pcr_wm_ng, .progressBar = FALSE));
  plot(psem_pcr_wm_ng, show="unstd"); # standardized not available
    
  # More complex path model: use linear regression (relatively similar estimates and p-values):
  psem_pcr_path_ng <- piecewiseSEM::psem(
    glm(education_years ~ age + location_bin, family=poisson(), data=d), # age + gender -> education
    lm(age ~ location_bin, data=d), # location + gender -> age
    lm(wm_norm ~ education_years + age, data=d), # age + gender + education -> wm
    lm(pcr ~ wm_norm + age + education_years + location_bin, data=d), # all -> pcr
    data=d);
  (summary_psem_pcr_path_ng <- summary(psem_pcr_path_ng, .progressBar = FALSE));
  plot(psem_pcr_path_ng, show="unstd"); # standardized not available
  ...
  tone_pcr_results <- list("icc_fam"=icc_pcr__fam ,"anova_fam"=anova_pcr__fam, "m_gen"=summary(m_pcr__gen), "m_location"=summary(m_pcr__location), # checks
                           "multreg"=m_pcr, "vif"=m_pcr_vif, # multiple regression model
                           ...
                           "no_gender"=list("multreg"=m_pcr_ng, "vif"=m_pcr_vif_ng,
                                            "med_wm__all"=list("mediation"=summary(med_pcr_wm_ng),
                                                               "piecewise"=list("linearreg"=list("model"=psem_pcr_wm_ng))),
                                            "path_model"=list("linearreg"=list("model"=psem_pcr_path_ng, "summary"=summary_psem_pcr_path_ng))),
                           "data"=d);
...
```
```{r fig.width=4, fig.height=4, fig.cap=capFig("Untransformed slopes with standard errors.")}
p1 <- sjPlot::plot_model(tone_pcr_results$multreg, show.values=TRUE, value.offset=.2, vline.color="gray20", transform=NULL); p1;
```
```{r fig.width=4, fig.height=4, fig.cap=capFig("Predicted values of pcr showing the interaction of*gender* and *age*.")}
p2 <- sjPlot::plot_model(tone_pcr_results$multreg, type="pred", terms=c("age", "gender")) + ylim(0,1); p2;
```
```{r fig.width=4, fig.height=4, fig.cap=capFig("Predicted values of pcr showing the interaction of *education_years* and *age*.")}
p3 <- sjPlot::plot_model(tone_pcr_results$multreg, type="pred", terms=c("education_years", "age")) + ylim(0,1) + xlab("education years"); p3;
```
...
```{r results='hide'}
# for the paper:
p1 <- sjPlot::plot_model(tone_pcr_results$multreg, show.values=TRUE, value.offset=.2, vline.color="gray20", transform=NULL) + ggtitle("A"); 
p2 <- sjPlot::plot_model(tone_pcr_results$multreg, type="pred", terms=c("age", "gender")) + ylim(0,1) + ggtitle("B");
p3 <- sjPlot::plot_model(tone_pcr_results$multreg, type="pred", terms=c("education_years", "age")) + ylim(0,1) + xlab("education years") + ggtitle("C");

cairo_ps("./paper/figures/Fig4.eps", width=3*5, height=1*5, onefile=TRUE, fallback_resolution=600);
grid.arrange(p1, p2, p3, nrow=1);
dev.off();
             
jpeg("./paper/figures/Fig4.jpg", units="in", width=3*5, height=1*5, quality=85, res=300);
grid.arrange(p1, p2, p3, nrow=1);
dev.off();
```
...
# med_gender__education:
g <- plot(tone_pcr_results$med_gender__education$piecewise$linearreg$model, show="unstd", 
          node_attrs = data.frame(shape = "circle", color = "black", fillcolor = "white", fixedsize=FALSE, fontsize=6), 
          edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6), 
          return=TRUE);
g_std <- coefs(tone_pcr_results$med_gender__education$piecewise$linearreg$model, standardize="scale"); # get the standardized coefficients and p-values
g_nodes <- get_node_df(g);
g_edges <- get_edge_df(g);
g_edges <- do.call(rbind, lapply(1:nrow(g_edges), function(i)
{
...
g_edges$estim <- as.numeric(g_edges$estim.nstd);
g_edges$label <- sprintf("%.1g%s", g_edges$estim, ifelse(g_edges$p.val<0.001, "***", ifelse(g_edges$p.val<0.01, "**", ifelse(g_edges$p.val<0.05, "*", ""))));
g <- set_node_attrs(g, "label",     c("educ",  "pcr",         "gender[M]"));
g <- set_node_attrs(g, "fillcolor", c("white", "lightyellow", "white"));
g <- set_node_attrs(g, "fontcolor", c("black", "black",       "gray60"));
g <- set_node_attrs(g, "color",     c("black", "black",       "gray60"));
g <- set_edge_attrs(g, "color",     ifelse(g_edges$style == "solid", ifelse(as.numeric(g_edges$estim) > 0, "darkorange", "deepskyblue"), "gray90"));
g <- set_edge_attrs(g, "fontcolor", ifelse(g_edges$style == "solid", ifelse(as.numeric(g_edges$estim) > 0, "firebrick", "navyblue"), "gray60"));
g <- set_edge_attrs(g, "label",     ifelse(g_edges$style == "solid", g_edges$label, ""));
render_graph(g);
DiagrammeR::export_graph(g, file_name="./paper/figures/Fig5A.pdf", file_type="pdf");


# med_age__education:
g <- plot(tone_pcr_results$med_age__education$piecewise$linearreg$model, show="unstd", 
          node_attrs = data.frame(shape = "circle", color = "black", fillcolor = "white", fixedsize=FALSE, fontsize=6), 
          edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6), 
          return=TRUE);
g_std <- coefs(tone_pcr_results$med_age__education$piecewise$linearreg$model, standardize="scale"); # get the standardized coefficients and p-values
g_nodes <- get_node_df(g);
g_edges <- get_edge_df(g);
g_edges <- do.call(rbind, lapply(1:nrow(g_edges), function(i)
{
...
g_edges$estim <- as.numeric(g_edges$estim.nstd);
g_edges$label <- sprintf("%.1g%s", g_edges$estim, ifelse(g_edges$p.val<0.001, "***", ifelse(g_edges$p.val<0.01, "**", ifelse(g_edges$p.val<0.05, "*", ""))));
g <- set_node_attrs(g, "label",     c("educ",  "pcr",         "age"));
g <- set_node_attrs(g, "fillcolor", c("white", "lightyellow", "white"));
g <- set_node_attrs(g, "fontcolor", c("black", "black",       "black"));
g <- set_node_attrs(g, "color",     c("black", "black",       "black"));
g <- set_edge_attrs(g, "color",     ifelse(g_edges$style == "solid", ifelse(as.numeric(g_edges$estim) > 0, "darkorange", "deepskyblue"), "gray90"));
...
g <- set_edge_attrs(g, "label",     ifelse(g_edges$style == "solid", g_edges$label, ""));
render_graph(g);
DiagrammeR::export_graph(g, file_name="./paper/figures/Fig5B.pdf", file_type="pdf");


# med_location__education:
g <- plot(tone_pcr_results$med_location__education$piecewise$linearreg$model, show="unstd", 
          node_attrs = data.frame(shape = "circle", color = "black", fillcolor = "white", fixedsize=FALSE, fontsize=6), 
          edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6), 
          return=TRUE);
g_std <- coefs(tone_pcr_results$med_location__education$piecewise$linearreg$model, standardize="scale"); # get the standardized coefficients and p-values
g_nodes <- get_node_df(g);
g_edges <- get_edge_df(g);
g_edges <- do.call(rbind, lapply(1:nrow(g_edges), function(i)
{
...
g_edges$estim <- as.numeric(g_edges$estim.nstd);
g_edges$label <- sprintf("%.1g%s", g_edges$estim, ifelse(g_edges$p.val<0.001, "***", ifelse(g_edges$p.val<0.01, "**", ifelse(g_edges$p.val<0.05, "*", ""))));
g <- set_node_attrs(g, "label",     c("educ",  "pcr",         "loc[B]"));
g <- set_node_attrs(g, "fillcolor", c("white", "lightyellow", "white"));
g <- set_node_attrs(g, "fontcolor", c("black", "black",       "black"));
g <- set_node_attrs(g, "color",     c("black", "black",       "black"));
g <- set_edge_attrs(g, "color",     ifelse(g_edges$style == "solid", ifelse(as.numeric(g_edges$estim) > 0, "darkorange", "deepskyblue"), "gray90"));
...
g <- set_edge_attrs(g, "label",     ifelse(g_edges$style == "solid", g_edges$label, ""));
render_graph(g);
DiagrammeR::export_graph(g, file_name="./paper/figures/Fig5C.pdf", file_type="pdf");


# complex:
g <- plot(tone_pcr_results$med_wm__all$piecewise$linearreg$model, show="unstd", 
          node_attrs = data.frame(shape = "circle", color = "black", fillcolor = "white", fixedsize=FALSE, fontsize=6), 
          edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6), 
          return=TRUE);
g_std <- coefs(tone_pcr_results$med_wm__all$piecewise$linearreg$model, standardize="scale"); # get the standardized coefficients and p-values
g_nodes <- get_node_df(g);
g_edges <- get_edge_df(g);
g_edges <- do.call(rbind, lapply(1:nrow(g_edges), function(i)
{
...
g_edges$estim <- as.numeric(g_edges$estim.nstd);
g_edges$label <- sprintf("%.1g%s", g_edges$estim, ifelse(g_edges$p.val<0.001, "***", ifelse(g_edges$p.val<0.01, "**", ifelse(g_edges$p.val<0.05, "*", ""))));
g <- set_node_attrs(g, "label",     c("wm",    "pcr",            "educ",  "gender[M]", "age"));
g <- set_node_attrs(g, "fillcolor", c("white", "lightyellow",    "white", "white",     "white"));
g <- set_node_attrs(g, "fontcolor", c("black", "black",          "black", "gray60",    "black"));
g <- set_node_attrs(g, "color",     c("black", "black",          "black", "gray60",    "black"));
g <- set_edge_attrs(g, "color",     ifelse(g_edges$style == "solid", ifelse(as.numeric(g_edges$estim) > 0, "darkorange", "deepskyblue"), "gray90"));
g <- set_edge_attrs(g, "fontcolor", ifelse(g_edges$style == "solid", ifelse(as.numeric(g_edges$estim) > 0, "firebrick", "navyblue"), "gray60"));
g <- set_edge_attrs(g, "label",     ifelse(g_edges$style == "solid", g_edges$label, ""));
render_graph(g);
DiagrammeR::export_graph(g, file_name="./paper/figures/Fig5D.pdf", file_type="pdf");
```
...
# unfortunately, because we have a Poisson model for education_years, we can't get standardized coefficients for it
# so, let's do a chimera: we report the standardized coefficients for all the other paths, but unstandardized for education_years...
g <- plot(tone_pcr_results$path_model$linearreg$model, show="unstd", 
          node_attrs = data.frame(shape = "circle", color = "black", fillcolor = "white", fixedsize=FALSE, fontsize=6), 
          edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6), 
          return=TRUE);
g_std <- coefs(tone_pcr_results$path_model$linearreg$model, standardize="scale"); # get the standardized coefficients and p-values
g_nodes <- get_node_df(g);
g_edges <- get_edge_df(g);
g_edges <- do.call(rbind, lapply(1:nrow(g_edges), function(i)
{
...
g_edges$label <- sprintf("%s%.1g%s%s",
                         ...
                         ifelse(g_edges$estim.std == "-", "]", ""),
                         ifelse(g_edges$p.val<0.001, "***", ifelse(g_edges$p.val<0.01, "**", ifelse(g_edges$p.val<0.05, "*", ""))));
g <- set_node_attrs(g, "label",     c("educ",  "age",   "gender[M]",  "wm",    "pcr",         "loc[B]"));
g <- set_node_attrs(g, "fillcolor", c("white", "white", "white",      "white", "lightyellow", "white"));
g <- set_node_attrs(g, "fontcolor", c("black", "black", "gray60",     "black", "black",       "black"));
g <- set_node_attrs(g, "color",     c("black", "black", "gray60",     "black", "black",       "black"));
g <- set_edge_attrs(g, "color",     ifelse(g_edges$style == "solid", ifelse(as.numeric(g_edges$estim) > 0, "darkorange", "deepskyblue"), "gray90"));
g <- set_edge_attrs(g, "fontcolor", ifelse(g_edges$style == "solid", ifelse(as.numeric(g_edges$estim) > 0, "firebrick", "navyblue"), "gray60"));
g <- set_edge_attrs(g, "label",     ifelse(g_edges$style == "solid", g_edges$label, ""));
render_graph(g);
DiagrammeR::export_graph(g, file_name="./paper/figures/Fig6A.pdf", file_type="pdf");
#DiagrammeR::export_graph(g, file_name="./paper/figures/Fig6A.png", file_type="png", width=10*72, height=10*72); # assuming 72 DPI
```
    ...
    !file.exists("./cached_results/tone_dpr_results.rds") ) # computationally expensive
{
  ...
  m_dpr_vif <- car::vif(m_dpr, type="predictor"); # all < 3, except gender (25)
  ...
  # multiple regression:
  m_dpr_full_ng <- lm(dpr ~ age * education_years * location_ab * wm_norm, data=d); summary(m_dpr_full_ng);
  m_dpr_ng <- update(m_dpr_full_ng, . ~ . -
                       age:education_years:location_ab:wm_norm - # 4-way interaction
                       education_years:location_ab:wm_norm - age:education_years:wm_norm - age:education_years:location_ab - age:location_ab:wm_norm - # 3-way interactions
                       age:location_ab - age:wm_norm - location_ab:wm_norm - education_years:wm_norm # 2-way interactions
  ); summary(m_dpr_ng); (anova_m_dpr_ng = anova(m_dpr_ng, m_dpr_full_ng));
  sjPlot::plot_model(m_dpr_ng, transform=NULL); 
  sjPlot::plot_model(m_dpr_ng, type="pred", terms=c("age", "education_years"));
  m_dpr_vif_ng <- car::vif(m_dpr_ng, type="predictor"); # all < 3, except for location_ab (25)
  
  ## working memory has a direct effect besides what education and age would predict:
  med_dpr__mediator_ng <-  lm(wm_norm ~ education_years + age, data=d); summary(med_dpr__mediator_ng);
  med_dpr__outcome_ng  <- lm(dpr ~ wm_norm + education_years + age, data=d); summary(med_dpr__outcome_ng); # mediation::mediate cannot do beta regression so let's use linear regression instead...
  summary(lm(dpr ~ wm_norm + education_years + age, data=d)); # ... and the estimates and p-values are very similar between the two
  med_dpr_wm_ng <- mediation::mediate(med_dpr__mediator_ng, med_dpr__outcome_ng, mediator="wm_norm", treat="education_years", outcome="dpr", sims=10000, robustSE=TRUE); summary(med_dpr_wm_ng); plot(med_dpr_wm_ng);

  psem_dpr_wm_ng <- piecewiseSEM::psem( # with indirect effect (aka partial mediation)
    lm(wm_norm ~ education_years + age, data=d),
    lm(dpr ~ wm_norm + education_years + age, data=d),
    data=d);
  (summary_psem_dpr_wm_ng <- summary(psem_dpr_wm_ng, .progressBar = FALSE));
  plot(psem_dpr_wm_ng, show="unstd"); # standardized not available
    
  # More complex path model: use linear regression (relatively similar estimates and p-values):
  psem_dpr_path_ng <- piecewiseSEM::psem(
    glm(education_years ~ age + location_bin, family=poisson(), data=d), # age + gender -> education
    lm(age ~ location_bin, data=d), # location + gender -> age
    lm(wm_norm ~ education_years + age, data=d), # age + gender + education -> wm
    lm(dpr ~ wm_norm + age + education_years + location_bin, data=d), # all -> dpr
    data=d);
  (summary_psem_dpr_path_ng <- summary(psem_dpr_path_ng, .progressBar = FALSE));
  plot(psem_dpr_path_ng, show="unstd"); # standardized not available

    
  # save them to file:
  tone_dpr_results <- list("icc_fam"=icc_dpr__fam, "m_gen"=summary(m_dpr__gen), "m_location"=summary(m_dpr__location), # checks
                           "multreg"=m_dpr, "vif"=m_dpr_vif, # multiple regression model
                           ...
                           "no_gender"=list("multreg"=m_dpr_ng, "vif"=m_dpr_vif_ng,
                                            "med_wm__all"=list("mediation"=summary(med_dpr_wm_ng),
                                                               "piecewise"=list("linearreg"=list("model"=psem_dpr_wm_ng))),
                                            "path_model"=list("linearreg"=list("model"=psem_dpr_path_ng, "summary"=summary_psem_dpr_path_ng))),
                           "data"=d);
...
# unfortunately, because we have a Poisson model for education_years, we can't get standardized coefficients for it
# so, let's do a chimera: we report the standardized coefficients for all the other paths, but unstandardized for education_years...
g <- plot(tone_dpr_results$path_model$model, show="unstd", 
          node_attrs = data.frame(shape = "circle", color = "black", fillcolor = "white", fixedsize=FALSE, fontsize=6), 
          edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6), 
          return=TRUE);
g_std <- coefs(tone_dpr_results$path_model$model, standardize="scale"); # get the standardized coefficients and p-values
g_nodes <- get_node_df(g);
g_edges <- get_edge_df(g);
g_edges <- do.call(rbind, lapply(1:nrow(g_edges), function(i)
{
...
g_edges$label <- sprintf("%s%.1g%s%s",
                         ...
                         ifelse(g_edges$estim.std == "-", "]", ""),
                         ifelse(g_edges$p.val<0.001, "***", ifelse(g_edges$p.val<0.01, "**", ifelse(g_edges$p.val<0.05, "*", ""))));
g <- set_node_attrs(g, "label",     c("educ",  "age",   "gender[M]",  "wm",    "dpr",         "loc[B]"));
g <- set_node_attrs(g, "fillcolor", c("white", "white", "white",      "white", "lightyellow", "white"));
g <- set_node_attrs(g, "fontcolor", c("black", "black", "gray60",     "black", "black",       "black"));
g <- set_node_attrs(g, "color",     c("black", "black", "gray60",     "black", "black",       "black"));
g <- set_edge_attrs(g, "color",     ifelse(g_edges$style == "solid", ifelse(as.numeric(g_edges$estim) > 0, "darkorange", "deepskyblue"), "gray90"));
...
g <- set_edge_attrs(g, "label",     ifelse(g_edges$style == "solid", g_edges$label, ""));
render_graph(g);
DiagrammeR::export_graph(g, file_name="./paper/figures/Fig6B.pdf", file_type="pdf");
#DiagrammeR::export_graph(g, file_name="./paper/figures/Fig6B.png", file_type="png", width=10*72, height=10*72); # assuming 72 DPI
```
    ...
    !file.exists("./cached_results/tone_pco_results.rds") ) # computationally expensive
{
  ...
  # let's switch the baseline model to this one (i.a, test the 2-way interactions):
  m_pco_full <- m_pco;
  m_pco <- update(m_pco_full, . ~ . - 
...
### Comparing the "original", "recoded" and "removed" results
...
```{r results='hide'}
if( FORCE_COMPUTE_ALL || 
    !file.exists("./cached_results/tone_pcx_results.rds") ) # computationally expensive
{
  ## Prepare for beta regression:
  d_all$pcx <- prepare_for_beta_regression(d_all$pc_tot_remv);
  
  # Keep only the relevant non-missing data:
  d <- na.omit(d_all[,c("pcx", "age", "gender", "gender_n", "education_years", "location_ab", "location_bin", "wm_norm")]);
  
  ## Just the complex path model using linear regression:
  psem_pcx_path <- piecewiseSEM::psem(
    glm(education_years ~ age + gender_n + location_bin, family=poisson(), data=d), # age + gender -> education
    lm(age ~ location_bin + gender_n, data=d), # location + gender -> age
    glm(gender_n ~ location_bin, family=binomial(), data=d), # location -> gender
    lm(wm_norm ~ education_years + gender_n + age, data=d), # age + gender + education -> wm
    lm(pcx ~ wm_norm + age + gender_n + education_years + location_bin, data=d), # all -> pcr
    data=d);
  (summary_psem_pcx_path <- summary(psem_pcx_path, .progressBar = FALSE));
  plot(psem_pcx_path, show="unstd"); # standardized not available
  
  
  # save them to file:
  tone_pcx_results <- list("path_model"=list("linearreg"=list("model"=psem_pcx_path, "summary"=summary_psem_pcx_path)),
                           "data"=d);
  saveRDS(tone_pcx_results, file=xzfile("./cached_results/tone_pcx_results.rds", compression=9));
} else
{
  tone_pcx_results <- readRDS(xzfile("./cached_results/tone_pcx_results.rds"));
}
```

```{r fig.width=5, fig.height=5, fig.cap=capFig("The complex path model for *pco*.")}
plot(tone_pco_results$path_model$linearreg$model, show="unstd", node_attrs = data.frame(shape = "rectangle", color = "black", fillcolor = "white", fontsize=6, width=0.65), edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6),digits=3);
```
```{r fig.width=5, fig.height=5, fig.cap=capFig("The complex path model for *pcr*.")}
plot(tone_pcr_results$path_model$linearreg$model, show="unstd", node_attrs = data.frame(shape = "rectangle", color = "black", fillcolor = "white", fontsize=6, width=0.65), edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6),digits=3);
```
```{r fig.width=5, fig.height=5, fig.cap=capFig("The complex path model for *pcx*.")}
plot(tone_pcx_results$path_model$linearreg$model, show="unstd", node_attrs = data.frame(shape = "rectangle", color = "black", fillcolor = "white", fontsize=6, width=0.65), edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6),digits=3);
```
...
```{r results='hide'}
if( FORCE_COMPUTE_ALL || 
    !file.exists("./cached_results/tone_dpx_results.rds") ) # computationally expensive
{
  ## Prepare for beta regression:
  d_all$dpx <- prepare_for_beta_regression(d_all$dprime_remv);
  
  # Keep only the relevant non-missing data:
  d <- na.omit(d_all[,c("dpx", "age", "gender", "gender_n", "education_years", "location_ab", "location_bin", "wm_norm")]);
  
  ## Just the complex path model using linear regression:
  psem_dpx_path <- piecewiseSEM::psem(
    glm(education_years ~ age + gender_n + location_bin, family=poisson(), data=d), # age + gender -> education
    lm(age ~ location_bin + gender_n, data=d), # location + gender -> age
    glm(gender_n ~ location_bin, family=binomial(), data=d), # location -> gender
    lm(wm_norm ~ education_years + gender_n + age, data=d), # age + gender + education -> wm
    lm(dpx ~ wm_norm + age + gender_n + education_years + location_bin, data=d), # all -> pcr
    data=d);
  (summary_psem_dpx_path <- summary(psem_dpx_path, .progressBar = FALSE));
  plot(psem_dpx_path, show="unstd"); # standardized not available
  
  
  # save them to file:
  tone_dpx_results <- list("path_model"=list("model"=psem_dpx_path, "summary"=summary_psem_dpx_path),
                           "data"=d);
  saveRDS(tone_dpx_results, file=xzfile("./cached_results/tone_dpx_results.rds", compression=9));
} else
{
  tone_dpx_results <- readRDS(xzfile("./cached_results/tone_dpx_results.rds"));
}
```

```{r fig.width=5, fig.height=5, fig.cap=capFig("The complex path model for *dpo*.")}
plot(tone_dpo_results$path_model$model, show="unstd", node_attrs = data.frame(shape = "rectangle", color = "black", fillcolor = "white", fontsize=6, width=0.65), edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6),digits=3);
```
```{r fig.width=5, fig.height=5, fig.cap=capFig("The complex path model for *dpr*.")}
plot(tone_dpr_results$path_model$model, show="unstd", node_attrs = data.frame(shape = "rectangle", color = "black", fillcolor = "white", fontsize=6, width=0.65), edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6),digits=3);
```
```{r fig.width=5, fig.height=5, fig.cap=capFig("The complex path model for *dpx*.")}
plot(tone_dpx_results$path_model$model, show="unstd", node_attrs = data.frame(shape = "rectangle", color = "black", fillcolor = "white", fontsize=6, width=0.65), edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6),digits=3);
```
...
# tone_pco_results
g <- plot(tone_pco_results$path_model$linearreg$model, show="unstd", 
          node_attrs = data.frame(shape = "circle", color = "black", fillcolor = "white", fixedsize=FALSE, fontsize=6), 
          edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6), 
          return=TRUE);
g_std <- coefs(tone_pco_results$path_model$linearreg$model, standardize="scale"); # get the standardized coefficients and p-values
g_nodes <- get_node_df(g);
g_edges <- get_edge_df(g);
g_edges <- do.call(rbind, lapply(1:nrow(g_edges), function(i)
{
...
g_edges$estim <- ifelse(g_edges$estim.std != "-", as.numeric(g_edges$estim.std), as.numeric(g_edges$estim.nstd));
g_edges$label <- sprintf("%s%.3g%s%s",
                         ifelse(g_edges$estim.std == "-", "[", ""),
                         g_edges$estim, 
                         ifelse(g_edges$estim.std == "-", "]", ""),
                         ifelse(g_edges$p.val<0.001, "***", ifelse(g_edges$p.val<0.01, "**", ifelse(g_edges$p.val<0.05, "*", ""))));
g <- set_node_attrs(g, "label",     c("educ",  "age",   "gender[M]",  "wm",    "pcr(O)",      "loc[B]"));
g <- set_node_attrs(g, "fillcolor", c("white", "white", "white",      "white", "lightyellow", "white"));
g <- set_node_attrs(g, "fontcolor", c("black", "black", "gray60",     "black", "black",       "black"));
g <- set_node_attrs(g, "color",     c("black", "black", "gray60",     "black", "black",       "black"));
g <- set_edge_attrs(g, "color",     ifelse(g_edges$style == "solid", ifelse(as.numeric(g_edges$estim) > 0, "darkorange", "deepskyblue"), "gray90"));
...
g <- set_edge_attrs(g, "label",     ifelse(g_edges$style == "solid", g_edges$label, ""));
render_graph(g);
DiagrammeR::export_graph(g, file_name="./paper/figures/FigS11A.pdf", file_type="pdf");
#DiagrammeR::export_graph(g, file_name="./paper/figures/FigS11A.png", file_type="png", width=10*72, height=10*72); # assuming 72 DPI
...
g_edges$label <- sprintf("%s%.3g%s%s",
                         ...
                         ifelse(g_edges$estim.std == "-", "]", ""),
                         ifelse(g_edges$p.val<0.001, "***", ifelse(g_edges$p.val<0.01, "**", ifelse(g_edges$p.val<0.05, "*", ""))));
g <- set_node_attrs(g, "label",     c("educ",  "age",   "gender[M]",  "wm",    "pcr(R)",      "loc[B]"));
g <- set_node_attrs(g, "fillcolor", c("white", "white", "white",      "white", "lightyellow", "white"));
g <- set_node_attrs(g, "fontcolor", c("black", "black", "gray60",     "black", "black",       "black"));
g <- set_node_attrs(g, "color",     c("black", "black", "gray60",     "black", "black",       "black"));
g <- set_edge_attrs(g, "color",     ifelse(g_edges$style == "solid", ifelse(as.numeric(g_edges$estim) > 0, "darkorange", "deepskyblue"), "gray90"));
...
g <- set_edge_attrs(g, "label",     ifelse(g_edges$style == "solid", g_edges$label, ""));
render_graph(g);
DiagrammeR::export_graph(g, file_name="./paper/figures/FigS11B.pdf", file_type="pdf");
#DiagrammeR::export_graph(g, file_name="./paper/figures/FigS11B.png", file_type="png", width=10*72, height=10*72); # assuming 72 DPI

# tone_pcx_results
g <- plot(tone_pcx_results$path_model$linearreg$model, show="unstd", 
          node_attrs = data.frame(shape = "circle", color = "black", fillcolor = "white", fixedsize=FALSE, fontsize=6), 
          edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6), 
          return=TRUE);
g_std <- coefs(tone_pcx_results$path_model$linearreg$model, standardize="scale"); # get the standardized coefficients and p-values
g_nodes <- get_node_df(g);
g_edges <- get_edge_df(g);
g_edges <- do.call(rbind, lapply(1:nrow(g_edges), function(i)
{
...
g_edges$label <- sprintf("%s%.3g%s%s",
                         ...
                         ifelse(g_edges$estim.std == "-", "]", ""),
                         ifelse(g_edges$p.val<0.001, "***", ifelse(g_edges$p.val<0.01, "**", ifelse(g_edges$p.val<0.05, "*", ""))));
g <- set_node_attrs(g, "label",     c("educ",  "age",   "gender[M]",  "wm",    "pcr(X)",      "loc[B]"));
g <- set_node_attrs(g, "fillcolor", c("white", "white", "white",      "white", "lightyellow", "white"));
g <- set_node_attrs(g, "fontcolor", c("black", "black", "gray60",     "black", "black",       "black"));
g <- set_node_attrs(g, "color",     c("black", "black", "gray60",     "black", "black",       "black"));
g <- set_edge_attrs(g, "color",     ifelse(g_edges$style == "solid", ifelse(as.numeric(g_edges$estim) > 0, "darkorange", "deepskyblue"), "gray90"));
...
g <- set_edge_attrs(g, "label",     ifelse(g_edges$style == "solid", g_edges$label, ""));
render_graph(g);
DiagrammeR::export_graph(g, file_name="./paper/figures/FigS11C.pdf", file_type="pdf");
#DiagrammeR::export_graph(g, file_name="./paper/figures/FigS11C.png", file_type="png", width=10*72, height=10*72); # assuming 72 DPI

# tone_dpo_results
g <- plot(tone_dpo_results$path_model$model, show="unstd", 
          node_attrs = data.frame(shape = "circle", color = "black", fillcolor = "white", fixedsize=FALSE, fontsize=6), 
          edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6), 
          return=TRUE);
g_std <- coefs(tone_dpo_results$path_model$model, standardize="scale"); # get the standardized coefficients and p-values
g_nodes <- get_node_df(g);
g_edges <- get_edge_df(g);
g_edges <- do.call(rbind, lapply(1:nrow(g_edges), function(i)
{
...
g_edges$label <- sprintf("%s%.3g%s%s",
                         ...
                         ifelse(g_edges$estim.std == "-", "]", ""),
                         ifelse(g_edges$p.val<0.001, "***", ifelse(g_edges$p.val<0.01, "**", ifelse(g_edges$p.val<0.05, "*", ""))));
g <- set_node_attrs(g, "label",     c("educ",  "age",   "gender[M]",  "wm",    "d′(O)",       "loc[B]"));
g <- set_node_attrs(g, "fillcolor", c("white", "white", "white",      "white", "lightyellow", "white"));
g <- set_node_attrs(g, "fontcolor", c("black", "black", "gray60",     "black", "black",       "black"));
g <- set_node_attrs(g, "color",     c("black", "black", "gray60",     "black", "black",       "black"));
g <- set_edge_attrs(g, "color",     ifelse(g_edges$style == "solid", ifelse(as.numeric(g_edges$estim) > 0, "darkorange", "deepskyblue"), "gray90"));
...
g <- set_edge_attrs(g, "label",     ifelse(g_edges$style == "solid", g_edges$label, ""));
render_graph(g);
DiagrammeR::export_graph(g, file_name="./paper/figures/FigS11D.pdf", file_type="pdf");
#DiagrammeR::export_graph(g, file_name="./paper/figures/FigS11D.png", file_type="png", width=10*72, height=10*72); # assuming 72 DPI
...
g_edges$label <- sprintf("%s%.3g%s%s",
                         ...
                         ifelse(g_edges$estim.std == "-", "]", ""),
                         ifelse(g_edges$p.val<0.001, "***", ifelse(g_edges$p.val<0.01, "**", ifelse(g_edges$p.val<0.05, "*", ""))));
g <- set_node_attrs(g, "label",     c("educ",  "age",   "gender[M]",  "wm",    "d′(R)",       "loc[B]"));
g <- set_node_attrs(g, "fillcolor", c("white", "white", "white",      "white", "lightyellow", "white"));
g <- set_node_attrs(g, "fontcolor", c("black", "black", "gray60",     "black", "black",       "black"));
g <- set_node_attrs(g, "color",     c("black", "black", "gray60",     "black", "black",       "black"));
g <- set_edge_attrs(g, "color",     ifelse(g_edges$style == "solid", ifelse(as.numeric(g_edges$estim) > 0, "darkorange", "deepskyblue"), "gray90"));
...
g <- set_edge_attrs(g, "label",     ifelse(g_edges$style == "solid", g_edges$label, ""));
render_graph(g);
DiagrammeR::export_graph(g, file_name="./paper/figures/FigS11E.pdf", file_type="pdf");
#DiagrammeR::export_graph(g, file_name="./paper/figures/FigS11E.png", file_type="png", width=10*72, height=10*72); # assuming 72 DPI

# tone_dpx_results
g <- plot(tone_dpx_results$path_model$model, show="unstd", 
          node_attrs = data.frame(shape = "circle", color = "black", fillcolor = "white", fixedsize=FALSE, fontsize=6), 
          edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6), 
          return=TRUE);
g_std <- coefs(tone_dpx_results$path_model$model, standardize="scale"); # get the standardized coefficients and p-values
g_nodes <- get_node_df(g);
g_edges <- get_edge_df(g);
g_edges <- do.call(rbind, lapply(1:nrow(g_edges), function(i)
{
...
g_edges$label <- sprintf("%s%.3g%s%s",
                         ...
                         ifelse(g_edges$estim.std == "-", "]", ""),
                         ifelse(g_edges$p.val<0.001, "***", ifelse(g_edges$p.val<0.01, "**", ifelse(g_edges$p.val<0.05, "*", ""))));
g <- set_node_attrs(g, "label",     c("educ",  "age",   "gender[M]",  "wm",    "d′(X)",       "loc[B]"));
g <- set_node_attrs(g, "fillcolor", c("white", "white", "white",      "white", "lightyellow", "white"));
g <- set_node_attrs(g, "fontcolor", c("black", "black", "gray60",     "black", "black",       "black"));
g <- set_node_attrs(g, "color",     c("black", "black", "gray60",     "black", "black",       "black"));
g <- set_edge_attrs(g, "color",     ifelse(g_edges$style == "solid", ifelse(as.numeric(g_edges$estim) > 0, "darkorange", "deepskyblue"), "gray90"));
...
g <- set_edge_attrs(g, "label",     ifelse(g_edges$style == "solid", g_edges$label, ""));
render_graph(g);
DiagrammeR::export_graph(g, file_name="./paper/figures/FigS11F.pdf", file_type="pdf");
#DiagrammeR::export_graph(g, file_name="./paper/figures/FigS11F.png", file_type="png", width=10*72, height=10*72); # assuming 72 DPI
```
...
## Appendix I: technical details about the methods
...
## Appendix II: % correct responses vs SDT
...
## Appendix III: excluding *gender* from the analyses

Here, we look in mode detail at *gender*: as one reviewer expressed some doubts about this variable, its role in our models, and its apparently high pairwise correlations with other predictors, we decided to investigate, on the one hand, if there are any multicollinearity issues in the multiple regressions models we use and, on the other, to see if excluding it from the analyses has any noticeable effects on the results.
As detailed below, given that it does not, we limit our investigation to the two primary measures only.


### % correct total (recode)

As a reminder, this is the manually simplified multiple regression model that included *gender*:
```{r}
summary(tone_pcr_results$multreg);
```

The (Generalized) VIFs for this model are:
```{r}
pander(tone_pcr_results$vif[,c("GVIF", "Df", "GVIF^(1/(2*Df))", "Interacts With")]);
```
and are all < 3 except for *gender* and (to a much lesser degree) *education_years*, both interacting with *age*. However, the adjusted generalized standard error inflation factors (aGSIFs; the `GVIF^(1/(2*Df))`) are all lower than $\sqrt{3} \approx$  `r round(sqrt(3),2)`, suggesting that there is no multicollinearity to worry about (see @fox_generalized_1992, as cited in https://www.bookdown.org/rwnahhas/RMPH/mlr-collinearity.html).
Nevertheless, we have also fitted the relevant models excluding it.

The VIFs of the original multiple regression (reminder):
```{r}
pander(tone_pcr_results$vif[,c("GVIF", "Df", "GVIF^(1/(2*Df))", "Interacts With")]);
```

The multiple regression (manually simplified) without *gender*:
```{r}
summary(tone_pcr_results$no_gender$multreg);
```
```{r fig.width=4, fig.height=4, fig.cap=capFig("Untransformed slopes with standard errors.")}
sjPlot::plot_model(tone_pcr_results$no_gender$multreg, show.values=TRUE, value.offset=.2, vline.color="gray20", transform=NULL);
```
```{r fig.width=4, fig.height=4, fig.cap=capFig("Predicted values of pcr showing the interaction of *education_years* and *age*.")}
sjPlot::plot_model(tone_pcr_results$no_gender$multreg, type="pred", terms=c("education_years", "age")) + ylim(0,1) + xlab("education years");
```
The (Generalized) VIFs for this model are:
```{r}
pander(tone_pcr_results$no_gender$vif[,c("GVIF", "Df", "GVIF^(1/(2*Df))", "Interacts With")]);
```
and, as expected, all < 3.

The relevant mediation and path models now are:

```{r}
summary(tone_pcr_results$no_gender$med_wm__all$piecewise$linearreg$model, .progressBar = FALSE);
```
```{r fig.width=5, fig.height=5, fig.cap=capFig("Mediation model of *age*, *years of education* and *pcr* showing the *un*standardized coefficients (fitted using `piecewiseSEM::psem()`).")}
plot(tone_pcr_results$no_gender$med_wm__all$piecewise$linearreg$model, show="unstd", node_attrs = data.frame(shape = "rectangle", color = "black", fillcolor = "white", fontsize=8, width=1), edge_attrs = data.frame(style = "solid", color = "gray80"),digits=2);
```

```{r}
summary(tone_pcr_results$no_gender$path_model$linearreg$model, .progressBar = FALSE);
```
```{r fig.width=5, fig.height=5, fig.cap=capFig("The complex path model for *pcr* showing the *un*standardized coefficients (fitted using `piecewiseSEM::psem()`).")}
plot(tone_pcr_results$no_gender$path_model$linearreg$model, show="unstd", node_attrs = data.frame(shape = "rectangle", color = "black", fillcolor = "white", fontsize=6, width=0.65), edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6),digits=2);
```

It can be seen that the results are virtually identical.
...
The (Generalized) VIFs for this model are:
```{r}
pander(tone_dpr_results$vif[,c("GVIF", "Df", "GVIF^(1/(2*Df))", "Interacts With")]);
```
and are all < 3 except for *gender*, interacting with *education_years*, but all aGSIFs are, again, all lower than $\sqrt{3} \approx$  `r round(sqrt(3),2)`, suggesting that there is no multicollinearity to worry about.
As above, we also fit the relevant models excluding *gender*.
...
The multiple regression (manually simplified) without *gender*:
```{r}
summary(tone_dpr_results$no_gender$multreg);
```
```{r fig.width=4, fig.height=4, fig.cap=capFig("Untransformed slopes with standard errors.")}
sjPlot::plot_model(tone_dpr_results$no_gender$multreg, show.values=TRUE, value.offset=.2, vline.color="gray20", transform=NULL);
```
```{r fig.width=4, fig.height=4, fig.cap=capFig("Predicted values of dpr showing the interaction of *education_years* and *age*.")}
sjPlot::plot_model(tone_dpr_results$no_gender$multreg, type="pred", terms=c("education_years", "age")) + ylim(0,6) + xlab("education years");
```
The (Generalized) VIFs for this model are:
```{r}
pander(tone_dpr_results$no_gender$vif[,c("GVIF", "Df", "GVIF^(1/(2*Df))", "Interacts With")]);
```
and, as expected, all < 3 except for *location_ab*, but whose aGVIF is < 2.

The relevant mediation and path models now are:

```{r}
summary(tone_dpr_results$no_gender$med_wm__all$piecewise$linearreg$model, .progressBar = FALSE);
```
```{r fig.width=5, fig.height=5, fig.cap=capFig("Mediation model of *age*, *years of education* and *dpr* showing the *un*standardized coefficients (fitted using `piecewiseSEM::psem()`).")}
plot(tone_dpr_results$no_gender$med_wm__all$piecewise$linearreg$model, show="unstd", node_attrs = data.frame(shape = "rectangle", color = "black", fillcolor = "white", fontsize=8, width=1), edge_attrs = data.frame(style = "solid", color = "gray80"),digits=2);
```

```{r}
summary(tone_dpr_results$no_gender$path_model$linearreg$model, .progressBar = FALSE);
```
```{r fig.width=5, fig.height=5, fig.cap=capFig("The complex path model for *dpr* showing the *un*standardized coefficients (fitted using `piecewiseSEM::psem()`).")}
plot(tone_dpr_results$no_gender$path_model$linearreg$model, show="unstd", node_attrs = data.frame(shape = "rectangle", color = "black", fillcolor = "white", fontsize=6, width=0.65), edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6),digits=2);
```

It can be seen that the results are virtually identical.


### Forcing the same number of males and females

The same reviewer expressed skepticism about our results because, quoting, "you have double the amount of women than men, which might be why [...] find an effect of female gender (mediated by other predictors)".
This can be tested in various ways, one of the simplest is downsampling (randomly) so that we have the same number of males and females in the dataset.
We focus only on the % correct total (recode) for illustration purposes.

The subsample has very similar distributions to the original sample, but now having a perfectly balanced gender distribution:

```{r}
d_all$pcr <- prepare_for_beta_regression(d_all$pc_tot_recd);
cat("Gender distribution (original):\n");
table(d_all$gender);

set.seed(42); # ensure reproducibility
d_subset <- d_all[ c(which(d_all$gender == "M"), sample(which(d_all$gender == "F"), replace=FALSE, size=sum(d_all$gender == "M"))), ];
cat("Gender distribution (subsample):\n");
table(d_subset$gender);

cat("Gender by location distribution (original):\n");
round(100*prop.table(table(d_all$gender, d_all$location)),1); # roughly 50:50 across A and B
cat("Gender by location distribution (subsample):\n");
round(100*prop.table(table(d_subset$gender, d_subset$location)),1); # roughly 50:50 across A and B and genders

ggplot(d_all, aes(x=age, fill=gender)) + geom_histogram(color="white", alpha=0.5) + ggtitle("Gender by age distribution (original)"); # very similar age distribution across genders
ggplot(d_subset, aes(x=age, fill=gender)) + geom_histogram(color="white", alpha=0.5) + ggtitle("Gender by age distribution (subsample)"); # very similar age distribution across genders

ggplot(d_all, aes(x=education_years, fill=gender)) + geom_histogram(color="white", alpha=0.5) + ggtitle("Gender by education distribution (original)"); # very similar age distribution across genders
ggplot(d_subset, aes(x=education_years, fill=gender)) + geom_histogram(color="white", alpha=0.5) + ggtitle("Gender by education distribution (subsample)"); # very similar age distribution across genders

# -> very similar to the original dataset

# let's redo the regression, mediations and path model...
if( FORCE_COMPUTE_ALL || 
    !file.exists("./cached_results/tone_pcr_subsample.rds") ) # computationally expensive
{
  # The data:
  d <- na.omit(d_subset[,c("pcr", "age", "gender", "gender_n", "education_years", "location_ab", "location_bin", "wm_norm")]);
  
  ## Multiple regression:
  # Manual simplification starting with the full model:
  beta_ctrl_s  <- glmmTMBControl(optCtrl=list(iter.max=1e8,eval.max=1e8));
  m_pcr_full_s <- glmmTMB(pcr ~ age * gender * education_years * location_ab * wm_norm, family=beta_family(), control=beta_ctrl_s, data=d); summary(m_pcr_full_s);
  m_pcr_s <- update(m_pcr_full_s, . ~ . - 
                    age:gender:education_years:location_ab:wm_norm - # 5-way interaction
                    age:gender:location_ab:wm_norm - age:education_years:location_ab:wm_norm - gender:education_years:location_ab:wm_norm - age:gender:education_years:wm_norm - age:gender:education_years:location_ab - # 4-way interactions
                    gender:location_ab:wm_norm - education_years:location_ab:wm_norm - age:location_ab:wm_norm - age:gender:location_ab - age:gender:wm_norm - 
                    gender:education_years:wm_norm - age:education_years:wm_norm - age:gender:education_years - age:education_years:location_ab - gender:education_years:location_ab - # 3-way interactions
                    age:wm_norm - gender:location_ab - age:location_ab - gender:wm_norm - education_years:wm_norm - location_ab:wm_norm - gender:education_years - education_years:location_ab # 2-way interactions
  ); summary(m_pcr_s); (anova_m_pcr_s = anova(m_pcr_s, m_pcr_full_s));
  sjPlot::plot_model(m_pcr_s, transform=NULL); 
  sjPlot::plot_model(m_pcr_s, type="pred", terms=c("age", "gender"));
  sjPlot::plot_model(m_pcr_s, type="pred", terms=c("education_years", "age"));
  m_pcr_vif_s <- car::vif(lm(formula(m_pcr_s), data=d), type="predictor"); # refit as a linear model as glmmTMB does not currently work and switching to linear should not be an issue in this case: all < 3 except for gender (14.1) and education_years (5.2)

  
  ## Mediation models:
  
  ## gender is a proxy for education:
  med_pcr__mediator_s <-  glm(education_years ~ gender_n, family=poisson(), data=d); summary(med_pcr__mediator_s); # poisson regression as education_years is a count
  med_pcr__outcome_s  <- lm(pcr ~ gender_n + education_years, data=d); summary(med_pcr__outcome_s); # mediation::mediate cannot do beta regression so let's use linear regression instead...
  summary(glmmTMB(pcr ~ gender_n + education_years, family=beta_family(), control=beta_ctrl_s, data=d)); # ... and the estimates and p-values are very similar between the two
  med_pcr_gender_s <- mediation::mediate(med_pcr__mediator_s, med_pcr__outcome_s, mediator="education_years", treat="gender_n", outcome="pcr", sims=10000, robustSE=TRUE); summary(med_pcr_gender_s); plot(med_pcr_gender_s); # no direct effect
  
  # use linear regression (very similar estimates and p-values):
  psem_pcr_gender_s <- piecewiseSEM::psem( # with indirect effect (aka partial mediation)
    glm(education_years ~ gender_n, family=poisson(), data=d),
    lm(pcr ~ gender_n + education_years, data=d),
    data=d);
  (summary_psem_pcr_gender_s <- summary(psem_pcr_gender_s, .progressBar = FALSE));
  plot(psem_pcr_gender_s, show="unstd"); # standardized not available for education_years
  psem_pcr_gender_full_s <- piecewiseSEM::psem( # just indirect effect (aka full mediation)
    glm(education_years ~ gender_n, family=poisson(), data=d),
    lm(pcr ~ education_years, data=d),
    data=d);
  (summary_psem_pcr_gender_full_s <- summary(psem_pcr_gender_full_s, .progressBar = FALSE));
  piecewiseSEM::dSep(psem_pcr_gender_full_s, .progressBar = FALSE); # p = 0.90
  plot(psem_pcr_gender_full_s, show="unstd"); # standardized not available for education_years
  (psem_anova_pcr_gender_s <- anova(psem_pcr_gender_s, psem_pcr_gender_full_s)); # p = 0.90
  # -> no direct effect of gender, but all is mediated through education
  
  
  ## age has an independent effect from education:
  med_pcr__mediator_s <-  glm(education_years ~ age, family=poisson(), data=d); summary(med_pcr__mediator_s); # poisson regression as education_years is a count
  med_pcr__outcome_s  <- lm(pcr ~ age + education_years, data=d); summary(med_pcr__outcome_s); # mediation::mediate cannot do beta regression so let's use linear regression instead...
  summary(glmmTMB(pcr ~ age + education_years, family=beta_family(), control=beta_ctrl_s, data=d)); # ... and the estimates and p-values are relatively (but not very) similar between the two
  med_pcr_age_s <- mediation::mediate(med_pcr__mediator_s, med_pcr__outcome_s, mediator="education_years", treat="age", outcome="pcr", sims=10000, robustSE=TRUE); summary(med_pcr_age_s); plot(med_pcr_age_s); # positive direct effect and negative indirect effect
  
  # use linear regression (relatively similar estimates and p-values):
  psem_pcr_age_s <- piecewiseSEM::psem( # with indirect effect (aka partial mediation)
    glm(education_years ~ age, family=poisson(), data=d),
    lm(pcr ~ age + education_years, data=d),
    data=d);
  (summary_psem_pcr_age_s <- summary(psem_pcr_age_s, .progressBar = FALSE));
  plot(psem_pcr_age_s, show="unstd"); # standardized not available for education_years
  psem_pcr_age_full_s <- piecewiseSEM::psem( # just indirect effect (aka full mediation)
    glm(education_years ~ age, family=poisson(), data=d),
    lm(pcr ~ education_years, data=d),
    data=d);
  (summary_psem_pcr_age_full_s <- summary(psem_pcr_age_full_s, .progressBar = FALSE));
  piecewiseSEM::dSep(psem_pcr_age_full_s, .progressBar = FALSE); # p = 0.0048 **
  plot(psem_pcr_age_full_s, show="unstd"); # standardized not available for education_years
  (psem_anova_pcr_age_s <- anova(psem_pcr_age_s, psem_pcr_age_full_s)); # p = 0.0047 **
  # -> there is a positive direct effect of age besides the mediated effect (older participants have less education)
  
  
  ## location has an independent effect from education:
  med_pcr__mediator_s <-  glm(education_years ~ location_bin, family=poisson(), data=d); summary(med_pcr__mediator_s); # poisson regression as education_years is a count
  med_pcr__outcome_s  <- lm(pcr ~ location_bin + education_years, data=d); summary(med_pcr__outcome_s); # mediation::mediate cannot do beta regression so let's use linear regression instead...
  summary(glmmTMB(pcr ~ location_bin + education_years, family=beta_family(), control=beta_ctrl_s, data=d)); # ... and the estimates and p-values are relatively (but not very) similar between the two
  med_pcr_location_s <- mediation::mediate(med_pcr__mediator_s, med_pcr__outcome_s, mediator="education_years", treat="location_bin", outcome="pcr", sims=10000, robustSE=TRUE); summary(med_pcr_location_s); plot(med_pcr_location_s); # negative direct and indirect effects
  
  # use linear regression (relatively similar estimates and p-values):
  psem_pcr_location_s <- piecewiseSEM::psem( # with indirect effect (aka partial mediation)
    glm(education_years ~ location_bin, family=poisson(), data=d),
    lm(pcr ~ location_bin + education_years, data=d),
    data=d);
  (summary_psem_pcr_location_s <- summary(psem_pcr_location_s, .progressBar = FALSE));
  plot(psem_pcr_location_s, show="unstd"); # standardized not available for education_years
  psem_pcr_location_full_s <- piecewiseSEM::psem( # just indirect effect (aka full mediation)
    glm(education_years ~ location_bin, family=poisson(), data=d),
    lm(pcr ~ education_years, data=d),
    data=d);
  (summary_psem_pcr_location_full_s <- summary(psem_pcr_location_full_s, .progressBar = FALSE));
  piecewiseSEM::dSep(psem_pcr_location_full_s, .progressBar = FALSE); # p = 9.29e-05 ***
  plot(psem_pcr_location_full_s, show="unstd"); # standardized not available for education_years
  (psem_anova_pcr_location_s <- anova(psem_pcr_location_s, psem_pcr_location_full_s)); # p = 1e-04 ***
  # -> the direct effect is needed
  
  ## working memory has a direct effect besides what education, age and gender would predict:
  med_pcr__mediator_s <-  lm(wm_norm ~ education_years + gender_n + age, data=d); summary(med_pcr__mediator_s);
  med_pcr__outcome_s  <- lm(pcr ~ wm_norm + education_years + gender_n + age, data=d); summary(med_pcr__outcome_s); # mediation::mediate cannot do beta regression so let's use linear regression instead...
  summary(glmmTMB(pcr ~ wm_norm + education_years + gender_n + age, family=beta_family(), control=beta_ctrl_s, data=d)); # ... and the estimates and p-values are very similar between the two
  med_pcr_wm_s <- mediation::mediate(med_pcr__mediator_s, med_pcr__outcome_s, mediator="wm_norm", treat="education_years", outcome="pcr", sims=10000, robustSE=TRUE); summary(med_pcr_wm_s); plot(med_pcr_wm_s);
  
  # use linear regression (relatively similar estimates and p-values):
  psem_pcr_wm_s <- piecewiseSEM::psem( # with indirect effect (aka partial mediation)
    lm(wm_norm ~ education_years + gender_n + age, data=d),
    lm(pcr ~ wm_norm + education_years + gender_n + age, data=d),
    data=d);
  (summary_psem_pcr_wm_s <- summary(psem_pcr_wm_s, .progressBar = FALSE));
  plot(psem_pcr_wm_s, show="unstd"); # standardized not available

  
  ## More complex path model:
  # use linear regression (relatively similar estimates and p-values):
  psem_pcr_path_s <- piecewiseSEM::psem(
    glm(education_years ~ age + gender_n + location_bin, family=poisson(), data=d), # age + gender -> education
    lm(age ~ location_bin + gender_n, data=d), # location + gender -> age
    glm(gender_n ~ location_bin, family=binomial(), data=d), # location -> gender
    lm(wm_norm ~ education_years + gender_n + age, data=d), # age + gender + education -> wm
    lm(pcr ~ wm_norm + age + gender_n + education_years + location_bin, data=d), # all -> pcr
    data=d);
  (summary_psem_pcr_path_s <- summary(psem_pcr_path_s, .progressBar = FALSE));
  plot(psem_pcr_path_s, show="unstd"); # standardized not available

  
  # save them to file:
  tone_pcr_subsample <- list("multreg"=m_pcr_s, "vif"=m_pcr_vif_s, # multiple regression model
                             "med_gender__education"=list("mediation"=summary(med_pcr_gender_s),
                                                          "piecewise"=list("linearreg"=list("model"=psem_pcr_gender_s, "dsep.p"=piecewiseSEM::dSep(psem_pcr_gender_full_s, .progressBar = FALSE)$P.Value, "anova"=psem_anova_pcr_gender_s))),
                             "med_age__education"=list("mediation"=summary(med_pcr_age_s),
                                                       "piecewise"=list("linearreg"=list("model"=psem_pcr_age_s, "dsep.p"=piecewiseSEM::dSep(psem_pcr_age_full_s, .progressBar = FALSE)$P.Value, "anova"=psem_anova_pcr_age_s))),
                             "med_location__education"=list("mediation"=summary(med_pcr_location_s),
                                                            "piecewise"=list("linearreg"=list("model"=psem_pcr_location_s, "dsep.p"=piecewiseSEM::dSep(psem_pcr_location_full_s, .progressBar = FALSE)$P.Value, "anova"=psem_anova_pcr_location_s))),
                             "med_wm__all"=list("mediation"=summary(med_pcr_wm_s),
                                                "piecewise"=list("linearreg"=list("model"=psem_pcr_wm_s))),
                             "path_model"=list("linearreg"=list("model"=psem_pcr_path_s, "summary"=summary_psem_pcr_path_s)),
                             "data"=d);
  saveRDS(tone_pcr_subsample, file=xzfile("./cached_results/tone_pcr_subsample.rds", compression=9));
} else
{
  tone_pcr_subsample <- readRDS(xzfile("./cached_results/tone_pcr_subsample.rds"));
}
```
...
```{r}
summary(tone_pcr_subsample$multreg);
```
```{r fig.width=4, fig.height=4, fig.cap=capFig("Untransformed slopes with standard errors.")}
p1 <- sjPlot::plot_model(tone_pcr_subsample$multreg, show.values=TRUE, value.offset=.2, vline.color="gray20", transform=NULL); p1;
```
```{r fig.width=4, fig.height=4, fig.cap=capFig("Predicted values of pcr showing the interaction of*gender* and *age*.")}
p2 <- sjPlot::plot_model(tone_pcr_subsample$multreg, type="pred", terms=c("age", "gender")) + ylim(0,1); p2;
```
```{r fig.width=4, fig.height=4, fig.cap=capFig("Predicted values of pcr showing the interaction of *education_years* and *age*.")}
p3 <- sjPlot::plot_model(tone_pcr_subsample$multreg, type="pred", terms=c("education_years", "age")) + ylim(0,1) + xlab("education years"); p3;
```
...
```{r}
tone_pcr_subsample$med_gender__education$mediation;
```
```{r}
summary(tone_pcr_subsample$med_gender__education$piecewise$linearreg$model, , .progressBar = FALSE);
```
```{r fig.width=5, fig.height=5, fig.cap=capFig("Mediation model of *gender*, *years of education* and *pcr* showing the *un*standardized coefficients (fitted using `piecewiseSEM::psem()`); dotted arrows are not significant.")}
plot(tone_pcr_subsample$med_gender__education$piecewise$linearreg$model, show="unstd", node_attrs = data.frame(shape = "rectangle", color = "black", fillcolor = "white", fontsize=8, width=1), edge_attrs = data.frame(style = "solid", color = "gray80"),digits=2);
```

`r sprintf("d-sep *p*=%s, model comparison *χ*^2^(%d)=%.2g, *p*=%s", scinot(tone_pcr_subsample$med_gender__education$piecewise$linearreg$dsep.p), as.numeric(tone_pcr_subsample$med_gender__education$piecewise$linearreg$anova[[1]][2,"Df.diff"]), as.numeric(tone_pcr_subsample$med_gender__education$piecewise$linearreg$anova[[1]][2,"Chisq.diff"]), scinot(as.numeric(tone_pcr_subsample$med_gender__education$piecewise$linearreg$anova[[1]][2,"P.value"])))`

#### Mediation: age → education → tone

```{r}
tone_pcr_subsample$med_age__education$mediation;
```
```{r}
summary(tone_pcr_subsample$med_age__education$piecewise$linearreg$model, .progressBar = FALSE);
```
```{r fig.width=5, fig.height=5, fig.cap=capFig("Mediation model of *age*, *years of education* and *pcr* showing the *un*standardized coefficients (fitted using `piecewiseSEM::psem()`).")}
plot(tone_pcr_subsample$med_age__education$piecewise$linearreg$model, show="unstd", node_attrs = data.frame(shape = "rectangle", color = "black", fillcolor = "white", fontsize=8, width=1), edge_attrs = data.frame(style = "solid", color = "gray80"),digits=2);
```

`r sprintf("d-sep *p*=%s, model comparison *χ*^2^(%d)=%.1f, *p*=%s", scinot(tone_pcr_subsample$med_age__education$piecewise$linearreg$dsep.p), as.numeric(tone_pcr_subsample$med_age__education$piecewise$linearreg$anova[[1]][2,"Df.diff"]), as.numeric(tone_pcr_subsample$med_age__education$piecewise$linearreg$anova[[1]][2,"Chisq.diff"]), tone_pcr_subsample$med_age__education$piecewise$linearreg$anova[[1]][2,"P.value"])`

#### Mediation: location → education → tone

```{r}
tone_pcr_subsample$med_location__education$mediation;
```
```{r}
summary(tone_pcr_subsample$med_location__education$piecewise$linearreg$model, .progressBar = FALSE);
```
```{r fig.width=5, fig.height=5, fig.cap=capFig("Mediation model of *age*, *years of education* and *pcr* showing the *un*standardized coefficients (fitted using `piecewiseSEM::psem()`).")}
plot(tone_pcr_subsample$med_location__education$piecewise$linearreg$model, show="unstd", node_attrs = data.frame(shape = "rectangle", color = "black", fillcolor = "white", fontsize=8, width=1), edge_attrs = data.frame(style = "solid", color = "gray80"),digits=2);
```

`r sprintf("d-sep *p*=%s, model comparison *χ*^2^(%d)=%.1f, *p*=%s", scinot(tone_pcr_subsample$med_location__education$piecewise$linearreg$dsep.p), as.numeric(tone_pcr_subsample$med_location__education$piecewise$linearreg$anova[[1]][2,"Df.diff"]), as.numeric(tone_pcr_subsample$med_location__education$piecewise$linearreg$anova[[1]][2,"Chisq.diff"]), tone_pcr_subsample$med_location__education$piecewise$linearreg$anova[[1]][2,"P.value"])`

#### Mediation: education, age, gender → working memory → tone

```{r}
tone_pcr_subsample$med_wm__all$mediation;
```
```{r}
summary(tone_pcr_subsample$med_wm__all$piecewise$linearreg$model);
```
```{r fig.width=5, fig.height=5, fig.cap=capFig("Mediation model of *education*, *age*, *gender*, *wm* and *pcr* showing the *un*standardized coefficients (fitted using `piecewiseSEM::psem()`).")}
plot(tone_pcr_subsample$med_wm__all$piecewise$linearreg$model, show="unstd", node_attrs = data.frame(shape = "rectangle", color = "black", fillcolor = "white", fontsize=6, width=0.65), edge_attrs = data.frame(style = "solid", color = "gray80"),digits=2);
```

#### Complex path model

```{r}
summary(tone_pcr_subsample$path_model$linearreg$model, .progressBar = FALSE);
```
```{r fig.width=5, fig.height=5, fig.cap=capFig("The complex path model for *pcr* showing the *un*standardized coefficients (fitted using `piecewiseSEM::psem()`).")}
plot(tone_pcr_subsample$path_model$linearreg$model, show="unstd", node_attrs = data.frame(shape = "rectangle", color = "black", fillcolor = "white", fontsize=6, width=0.65), edge_attrs = data.frame(style = "solid", color = "gray80", fontsize=6),digits=2);
```

#### Conclusions

It can be seen that, with the limitations imposed by the smaller sample size on the significance of some effects (e.g., the gender → age path in the complex path model) and the slight variation in the estimates due to the sampling procedure, the results are virtually identical. 
